...
class TripControllerTest {
    ...
    private Currency cur1 = new Currency();
    ...
    public void setUp() throws Exception {
        ...
        cur0.setExchangeRate(2.0);
        cur1.setId(2L);
        cur1.setAbreviation("USD");
        cur1.setExchangeRate(1.5);
        ...
        when(routeService.getRoute(12345L)).thenReturn(null);
        when(currencyService.getCurrency("EUR")).thenReturn(cur0);
        when(currencyService.getCurrency("USD")).thenReturn(cur1);
    }
    ...
    void givenBadValue_whenAdd_thenReturnError() throws Exception {
        //  Check bad number of seats
        mvc.perform(
                post("/trips/add").contentType(MediaType.APPLICATION_JSON)
                .param("numberOfSeatsAvailable", "16")
                .param("numberOfSeatsTotal", "12")
                .param("tripLengthTime", trip0.getTripLengthTime())
                .param("tripLengthKm", trip0.getTripLengthKm())
                .param("date", "2-4-2024")
                .param("time", "19:22:59")
        ...
        mvc.perform(
                post("/trips/add").contentType(MediaType.APPLICATION_JSON)
                .param("numberOfSeatsAvailable", "-1")
                .param("numberOfSeatsTotal", "-2")
                .param("tripLengthTime", trip0.getTripLengthTime())
                .param("tripLengthKm", trip0.getTripLengthKm())
                .param("date", "2-4-2024")
                .param("time", "19:22:59")
    ...
    @Test
    void givenSomeTripsWithNoCurrency_thenReturnTheTripsByRouteAndSetCurrencyToUSD() throws Exception {
        mvc.perform(
                get("/trips/listByRoute").contentType(MediaType.APPLICATION_JSON)
                .param("route", trip0.getRoute().getId().toString())
                .param("currency", ""))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].basePrice", is(19.36)))
                .andExpect(jsonPath("$[1].basePrice", is(12.18)));

        verify(currencyService, times(1)).getCurrency("USD");
        verify(tripService, times(1)).listTripsByRouteID(Mockito.any());
    }

    @Test
    void givenInvalidID_whenListByRoute_thenReturnError() throws Exception {
        mvc.perform(
                get("/trips/listByRoute").contentType(MediaType.APPLICATION_JSON)
                .param("route", "12345")
                .param("currency", "EUR"))
        ...
        verify(tripService, times(0)).listTripsByRouteID(Mockito.any());
